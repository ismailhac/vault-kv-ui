...
<template>
  ...
  >
    <div class="bg-gray-900 border border-red-900 rounded-lg w-full max-w-lg shadow-2xl flex flex-col max-h-[85vh] light:bg-white light:border-red-300">
      ...
      <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-800 shrink-0 light:border-gray-200">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="text-gray-500 text-xs mt-0.5 font-mono truncate light:text-gray-500">{{ path }}{{ isFolder ? '/' : '' }}</div>
        </div>
        <button class="text-gray-500 hover:text-gray-300 shrink-0 light:hover:text-gray-700" @click="emit('cancel')">✕</button>
      </div>

      <!-- Single secret body -->
      <div v-if="!isFolder" class="px-5 py-4 space-y-3">
        <p class="text-gray-400 text-xs light:text-gray-600">
      ...
      <!-- Folder body: loading -->
      <div v-else-if="loadingPaths" class="px-5 py-8 text-center text-gray-500 text-sm animate-pulse light:text-gray-400">
      ...
      <div v-else class="flex flex-col min-h-0 flex-1">
        ...
        <div class="flex items-center justify-between px-5 py-2 border-b border-gray-800 shrink-0 light:border-gray-200">
          ...
          <span class="text-gray-600 text-xs light:text-gray-500">{{ allPaths.length === 0 ? t('deleteConfirmModal.emptyFolder') : t('deleteConfirmModal.scrollDown') }}</span>
        </div>
        ...
        <div class="overflow-y-auto flex-1 px-2 py-2 space-y-1">
          <div v-for="group in tree" :key="group.folder">
            ...
            >
              ...
              <span class="text-yellow-300/80 text-xs font-mono light:text-yellow-700">{{ group.folder || '(root)' }}/</span>
              <span class="text-gray-600 text-[10px] ml-auto light:text-gray-500">{{ group.secrets.length }}</span>
            </div>
            <!-- Secrets -->
            <div
              v-for="secret in group.secrets"
              :key="`${group.folder}/${secret}`"
              class="flex items-center gap-2 pl-8 pr-3 py-1 rounded cursor-pointer hover:bg-gray-800/60 select-none light:hover:bg-gray-50"
            ...
            >
              ...
              <span
                ...
                :class="selectedPaths.has(group.folder ? `${group.folder}/${secret}` : secret) ? 'text-red-300 light:text-red-700' : 'text-gray-500 light:text-gray-600'"
              >{{ secret }}</span>
            </div>
          </div>
          <div v-if="allPaths.length === 0" class="text-center text-gray-600 text-xs py-4 light:text-gray-500">

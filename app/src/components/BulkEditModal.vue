...
<template>
  ...
  >
    <div class="bg-gray-900 border border-gray-700 rounded-lg w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl light:bg-white light:border-gray-200">
      <!-- Header -->
      <div class="flex items-center justify-between px-5 py-3 border-b border-gray-700 light:border-gray-200">
        <span class="text-white font-semibold text-sm light:text-gray-900">{{ t('bulkEditModal.title') }} <span class="text-green-400">{{ vault.currentMount }}</span></span>
      ...
      <div v-if="loadingPaths" class="flex-1 flex flex-col items-center justify-center py-16 gap-4">
        <div class="w-12 h-12 border-2 border-gray-700 border-t-green-400 rounded-full animate-spin light:border-gray-200 light:border-t-green-500"></div>
        <p class="text-gray-400 text-sm light:text-gray-600">{{ t('bulkEditModal.loadingSecrets') }}</p>
        <p class="text-gray-600 text-xs light:text-gray-500">{{ t('bulkEditModal.scanningMount') }} <span class="text-green-500">{{ vault.currentMount }}</span></p>
      </div>
      ...
      <div v-else class="overflow-auto flex-1 px-5 py-4 space-y-4">
        ...
        <div v-if="!previewReady">
          <!-- Preset selector -->
          <div class="mb-4">
            <label class="text-gray-400 text-xs block mb-2 light:text-gray-600">{{ t('bulkEditModal.projectPreset') }}</label>
          ...
          <div class="mb-4">
            <div class="flex items-center gap-2 mb-2">
              <label class="text-gray-400 text-xs light:text-gray-600">{{ t('bulkEditModal.pathToEdit') }}</label>
              <button
                type="button"
            ...
            <div class="flex gap-2">
              <select
                ...
                class="flex-1 px-3 py-2 bg-gray-950 border border-gray-700 text-green-300 rounded text-sm focus:outline-none focus:border-green-600 disabled:opacity-50 light:bg-white light:border-gray-300 light:text-green-700"
              >
          ...
          <!-- JSON input -->
          <p class="text-gray-400 text-xs mb-2 light:text-gray-600">
          ...
          <textarea
            v-model="bulkJson"
            class="w-full h-72 bg-gray-950 border border-gray-700 text-green-300 font-mono text-xs rounded p-3 resize-y focus:outline-none focus:border-green-600 light:bg-white light:border-gray-300 light:text-green-700"
        ...
        <div v-if="previewReady" class="space-y-4">
          <div class="flex items-center justify-between">
            <span class="text-gray-400 text-sm light:text-gray-600">{{ t('bulkEditModal.pathsModified', { paths: previews.length, modified: totalChanges }) }}</span>
            <div class="flex gap-2">
              <button
          ...
          <div
            ...
            :key="entry.path"
            class="border border-gray-700 rounded light:border-gray-200"
          >
            <div class="flex items-center justify-between px-4 py-2 bg-gray-800 rounded-t light:bg-gray-100">
      ...
      <div v-if="allApplied" class="px-5 py-3 border-t border-gray-700 text-green-400 text-sm text-center light:border-gray-200">

...
<template>
    ...
    <div class="flex items-center gap-2">
      <button
        ...
        :class="boolVal
          ...
          : 'bg-gray-700 border-gray-600 text-gray-300 hover:bg-gray-600 light:bg-gray-100 light:border-gray-300 light:text-gray-700 light:hover:bg-gray-200'"
  ...
  <template v-else-if="isInteger">
    <div class="flex items-center gap-1">
      <button type="button" class="px-1.5 py-0.5 text-xs bg-gray-700 hover:bg-gray-600 text-gray-200 rounded shrink-0 light:bg-gray-200 light:hover:bg-gray-300 light:text-gray-800" @click.stop="decrement">−</button>
      <input
        v-focus
        :value="modelValue"
        class="w-20 text-center bg-gray-800 border border-blue-500 text-gray-100 font-mono text-xs rounded px-2 py-0.5 focus:outline-none focus:border-blue-400 light:bg-gray-100 light:text-gray-900"
        @input="emit('update:modelValue', ($event.target as HTMLInputElement).value)"
        @keyup.enter="emit('confirm')"
        @keyup.escape="emit('cancel')"
        @click.stop
      />
      <button type="button" class="px-1.5 py-0.5 text-xs bg-gray-700 hover:bg-gray-600 text-gray-200 rounded shrink-0 light:bg-gray-200 light:hover:bg-gray-300 light:text-gray-800" @click.stop="increment">+</button>
    ...
    <div class="flex flex-col gap-0.5 flex-1 min-w-0">
      <div class="flex items-center gap-1">
        <input
          v-focus
          :value="modelValue"
          class="flex-1 min-w-0 bg-gray-800 border border-blue-500 text-gray-100 font-mono text-xs rounded px-2 py-0.5 focus:outline-none focus:border-blue-400 light:bg-gray-100 light:text-gray-900"
          @input="emit('update:modelValue', ($event.target as HTMLInputElement).value)"
          @keyup.enter="emit('confirm')"
          @keyup.escape="emit('cancel')"
          @click.stop

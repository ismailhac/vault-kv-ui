...
async function copyCurrentPath() {
  await navigator.clipboard.writeText(vault.currentPath)
...
const compareSeed = ref<{ source: string; target: string } | null>(null)

function onOpenCompare(payload: { source: string; target: string }) {
  showCreate.value = false
  compareSeed.value = payload
...
<template>
  ...
  <div class="flex items-center justify-between mb-4 gap-2 flex-wrap">
    ...
    <div class="flex items-center gap-2 min-w-0">
      ...
      <button
        class="p-1 rounded pointer transition-colors shrink-0"
        :class="pathCopied ? 'text-green-400 bg-green-900/30 light:text-green-600 light:bg-green-50' : 'text-gray-400 hover:text-gray-100 hover:bg-gray-700 light:hover:bg-gray-200 light:hover:text-gray-700'"
        :title="t('browserView.copyPath')"
        @click="copyCurrentPath"
      >
        <svg v-if="pathCopied" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
  ...
  <!-- Modals -->
  <CreateSecretModal v-if="showCreate" @close="showCreate = false" @open-compare="onOpenCompare" />
  <DeleteConfirmModal
    v-if="pendingDelete"
  ...
  <CompareModal v-if="showCompare" :initial-source="compareSeed?.source" :initial-target="compareSeed?.target" @close="showCompare = false; compareSeed = null" />

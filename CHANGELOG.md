...
### Fixes
- create-secret-propagation: preserve native JSON object type when merging nested keys into existing secret

## [1.10.1] — 2026-06-23

### Fixes
- create-secret-propagation: merge nested dotted keys into existing secret instead of overwriting
...
### Changed
- Version bump to verify npm publish pipeline
...
### Fixed
- update-badge-not-showing: Removed localStorage update cache entirely — the BFF already caches the npm registry response for 1 hour in memory, so the frontend no longer caches the result. This fixes the issue where a fresh install would see `{ hasUpdate: false }` cached from a prior session and never call the BFF again, hiding the update badge even when a newer version was available.
